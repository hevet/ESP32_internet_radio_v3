...
// Rozdzielczość wyświetlacza TFT (typowa dla ILI9488 to 480x320)
...
void tft_fillScreen(uint8_t r, uint8_t g, uint8_t b)
{
  tft_setAddrWindow(0, 0, TFT_WIDTH-1, TFT_HEIGHT-1);  // Ustawienie całego ekranu jako obszaru do rysowania
  DC_DATA;                                              // Przełączenie linii DC na tryb danych
  CS_ACTIVE;                                           // Aktywacja chip select

  static uint8_t line[TFT_WIDTH*3];                     // Bufor jednej linii w pikselach RGB (3 bajty na piksel)

  for(int x=0 ; x<TFT_WIDTH ; x++)
  {
    line[x*3]   = r;                                   // Ustawienie czerwonej składowej piksela
    line[x*3+1] = g;                                   // Ustawienie zielonej składowej piksela
    line[x*3+2] = b;                                   // Ustawienie niebieskiej składowej piksela
  }

  for(int y=0 ; y<TFT_HEIGHT ; y++)
  {
    spi.writeBytes(line, sizeof(line));                // Wysłanie całej linii pikseli do ekranu
    yield();                                           // Pozwolenie innym zadaniom na wykonanie (ESP8266/ESP32)
  }

  CS_IDLE;                                             // Dezaktywacja chip select po zakończeniu wysyłania
}
...
void volumeSet()
{
  ...
  tft_fillRect(40, 265, 50, 25, 0, 0, 0); // czarne tło pod zegar
